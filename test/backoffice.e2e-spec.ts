...
describe('Back office (e2e)', () => {
  ...
  it('lets platform staff create an operator admin for a tenant without one', async () => {
    const betzone = await prisma.casinoGroup.findUniqueOrThrow({
      where: { slug: 'betzone' },
    });
    ...
    await request(app.getHttpServer())
      .get(`/api/v1/backoffice/staff/operators?casinoGroupId=${betzone.id}`)
      .set('Authorization', `Bearer ${token}`)
      .expect(200)
      .expect((res) => {
    ...
    const created = await request(app.getHttpServer())
      .post(`/api/v1/backoffice/staff/operators?casinoGroupId=${betzone.id}`)
      .set('Authorization', `Bearer ${token}`)
      .send({
        email: 'ops@betzone-e2e.test',
        password: 'BetzoneOpE2e123!',
      })
      .expect(201);

    expect(created.body.email).toBe('ops@betzone-e2e.test');
    expect(created.body.passwordAutoGenerated).toBe(false);

    await request(app.getHttpServer())
      .post('/api/v1/backoffice/auth/login')
      .send({ email: 'ops@betzone-e2e.test', password: 'BetzoneOpE2e123!' })
      .expect(201);

    const operator = await prisma.staffUser.findUniqueOrThrow({
      where: { email: 'ops@betzone-e2e.test' },
    });
    await prisma.staffSession.deleteMany({ where: { staffUserId: operator.id } });
    await prisma.staffUser.delete({ where: { id: operator.id } });
  });
